...
const AdminUsers = () => {
  ...
  const [editUserFormData, setEditUserFormData] = useState({
    ...
    email: "",
    role: "Agent",
    status: "active" as unknown as any // Type assertion to avoid literal mismatch if strict
  });
  ...
  const createUserMutation = useMutation({
    mutationFn: (userData: Partial<ApiUser>) => api.createUser(userData),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['users'] });
      toast({ title: "Success", description: "User created successfully" });
      setIsAddUserOpen(false);
      setNewUser({ name: "", email: "", phone: "", password: "", role: "agent", department: "Sales" });
    },
    onError: (error: Error) => {
  ...
  const refreshData = () => {
    queryClient.invalidateQueries({ queryKey: ['users'] });
    toast({ title: "Refreshing", description: "Fetching latest user data..." });
  };
  ...
  const stats = {
    ...
    agents: users.filter(u => u.role === "agent" || u.role === "Agent").length
  };
  ...
  const getRoleBadge = (role: string) => {
    ...
    return (
      <Badge className={colors[r] || "bg-gray-100 text-gray-800"}>
  ...
  const exportToCSV = () => {
    // ... existing logic but using filteredUsers
    const csvContent = [
                ...
                >
                  ...
                  <div className="relative z-10 space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      >
                        <Badge className={`px-3 py-1 font-semibold shadow-sm ${user.status === 'active'
                      ...
                      <div>
                        ...
                        <div className="space-y-2 text-sm">
                          ...
                          <div>
                            ...
                            <span className="text-gray-900">{selectedUser._id}</span>
                          </div>
                        </div>
                      </div>
                    </div>
