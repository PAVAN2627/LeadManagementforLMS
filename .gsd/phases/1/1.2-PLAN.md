...
<task type="auto">
  ...
  <action>
    ...
    - Fields: name, email (unique), passwordHash (hash), role (enum: 'admin', 'manager', 'agent'), status (active/inactive), createdAt.
    - Export TypeScript interface `IUser`.
    - Export Mongoose model `User`.
    - *Note: Using lowercase roles 'admin', 'manager', 'agent' to match frontend conventions.*
  </action>
  <verify>grep "passwordHash" src/models/User.ts</verify>
